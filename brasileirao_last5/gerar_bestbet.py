...
'''
    ...
    <script>
        ...
        function atualizarContador() {
            ...
            // Habilita/desabilita botão "Publicar no App" baseado na seleção
            if (publishBtn) {
                publishBtn.disabled = (selecionadas === 0);
                publishBtn.style.opacity = (selecionadas === 0) ? '0.5' : '1';
                publishBtn.style.cursor = (selecionadas === 0) ? 'not-allowed' : 'pointer';
            }
        }
        ...
        function extrairBetsDaPagina() {
            ...
            campeonatoSections.forEach(section => {
                ...
                let leagueName = header ? header.textContent.trim() : '';
                
                // Remove informação de data/hora de atualização
                leagueName = leagueName.replace(/Atualizado:.*$/, '').trim();
