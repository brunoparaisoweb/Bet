#!/usr/bin/env python3
"""
Gerador de HTML para Ligue 1
Baseado no sistema do Brasileirão, adaptado para Ligue 1 2025/2026
"""
import json
from datetime import datetime
from sofascore_scraper_ligue1 import scrape_all_teams_sofascore, LIGUE1_TEAM_IDS
from config_ligue1 import PROXIMA_RODADA, CLASSIFICACAO_ATUAL
...
def calcular_creditos_time(team_name, ultimos_jogos, classificacao, h2h_data=None):
    ...
    for jogo in ultimos_jogos[:5]:
        ...
        home_score = int(jogo.get("placar_mandante", 0))
        away_score = int(jogo.get("placar_visitante", 0))
...
def normalizar_nome_time(nome):
    ...
    apelidos = {
        ...
        "Olympique de Marselha": ["Olympique de Marselha", "Marseille", "Olympique Marseille"],
...
def extrair_h2h_confronto_direto(time_mandante, time_visitante, debug=False):
    ...
    # Busca confrontos diretos no ogol.com.br (função async)
    jogos_h2h = asyncio.run(scrape_h2h_ogol_ligue1(time_mandante, time_visitante, debug=debug))
...
def gerar_html_LIGUE1():
    """
    Gera o HTML principal com análise da Ligue 1
    """
    print("=== GERANDO HTML Ligue 1 ===\n")
    
    # 1. Buscar últimos jogos de cada time
    print("1. Buscando últimos jogos do SofaScore...")
    dados_times = {}
    
    # Buscar dados de todos os times de uma vez
    print("   Buscando dados de todos os times...")
    all_data = scrape_all_teams_sofascore(debug=False)
    
    # Processar dados de cada time
    for team_name, jogos in all_data.items():
        # Filtra apenas jogos de 2026 (formato da data: DD/MM/YY)
        jogos_2026 = [j for j in jogos if j.get("data", "").endswith("/26")]
        
        dados_times[team_name] = {
            "ultimos_jogos": jogos_2026,
            "team_id": LIGUE1_TEAM_IDS[team_name]
        }
    ...
    # Salvar arquivo
    with open("LIGUE1_analysis.html", "w", encoding="utf-8") as f:
        f.write(html)
...
<head>
    ...
    <title>Ligue 1 2025/2026 - Análise</title>
    <style>
        body {
...
<body>
    ...
    <div class="sidebar">
        <h3>Próxima Rodada - Ligue 1 (Rodada 19)</h3>
"""
    
    # Próxima rodada na sidebar
    for jogo in PROXIMA_RODADA:
        # Simplifica os nomes
        home_short = jogo["mandante"][:3].upper() if len(jogo["mandante"]) > 10 else jogo["mandante"]
        away_short = jogo["visitante"][:3].upper() if len(jogo["visitante"]) > 10 else jogo["visitante"]
        
        # Processa data
        data_completa = jogo.get("data", "")
        data_short = data_completa[:5] if len(data_completa) >= 5 else data_completa
        
        # Usa campo 'hora' se disponível
...
"""
        ...
        for jogo in jogos:
            ...
            home_team = jogo.get("mandante", "")
            away_team = jogo.get("visitante", "")
            home_score = jogo.get("placar_mandante", 0)
            away_score = jogo.get("placar_visitante", 0)
            
            # Data do jogo (formato já vem como DD/MM/YY do scraper)
            date_str = jogo.get("data", "")
            if date_str:
