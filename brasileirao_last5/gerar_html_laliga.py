#!/usr/bin/env python3
"""
Gerador de HTML para LaLiga (Campeonato Espanhol)
Baseado no sistema da Premier League, adaptado para LaLiga 2025/2026
"""
import json
from datetime import datetime
from sofascore_scraper_laliga import scrape_sofascore_last5, LALIGA_TEAM_IDS
from config_laliga import PROXIMA_RODADA, CLASSIFICACAO_ATUAL
from ogol_scraper_laliga import scrape_h2h_ogol
...
def normalizar_nome_time(nome):
    ...
    """
    apelidos = {
        "Atlético Madrid": ["Atlético Madrid", "Atl. Madrid", "Atlético de Madrid", "Atletico Madrid"],
        "Atl. Madrid": ["Atlético Madrid", "Atl. Madrid", "Atlético de Madrid", "Atletico Madrid"],
        "Real Betis": ["Real Betis", "Betis"],
        "Betis": ["Real Betis", "Betis"],
        "Celta": ["Celta", "Celta de Vigo", "Celta Vigo"],
        "Celta de Vigo": ["Celta", "Celta de Vigo", "Celta Vigo"],
        ...
        "Barcelona": ["Barcelona", "Barça", "FC Barcelona"],
    }
...
def gerar_html_laliga():
    """
    Gera o HTML principal com análise da LaLiga
    """
    # 1. Buscar últimos jogos de cada time
    print("Buscando dados do SofaScore...")
    dados_times = {}
    
    for team_name, team_id in LALIGA_TEAM_IDS.items():
        # Evita duplicatas (usa nome completo, não abreviações)
        if team_name in ["Betis", "Atl. Madrid", "R. Sociedad", "Athletic Bilbao", "Atlético de Madrid", "Celta de Vigo"]:
            continue
    ...
    # Salvar arquivo
    with open("laliga_analysis.html", "w", encoding="utf-8") as f:
        f.write(html)
...
<head>
    ...
    <title>LaLiga 2025/2026 - Análise</title>
    <style>
        body {
...
"""
        ...
        for jogo in jogos:
            ...
            # Usa função de correspondência para aceitar apelidos
...
"""
    ...
    html += """        </table>
        ...
        <h3 style="margin-top: 12px; color: #FF4C00; font-size: 0.85em;">BETs Recomendadas</h3>
"""
