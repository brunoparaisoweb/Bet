#!/usr/bin/env python3
"""
Gerador de HTML para Serie A (Itália)
Baseado no sistema da Premier League, adaptado para Serie A 2025/2026
"""
import json
...
def normalizar_nome_time(nome):
    ...
    """
    apelidos = {
        "Internazionale": ["Internazionale", "Inter", "Inter Milan", "Inter de Milão"],
        "Inter": ["Internazionale", "Inter", "Inter Milan", "Inter de Milão"],
        "Como": ["Como", "Como 1907"],
...
def extrair_h2h_confronto_direto(time_mandante, time_visitante, debug=False):
    ...
    jogos_h2h = scrape_h2h_ogol_seria(time_mandante, time_visitante, debug=debug)
...
def gerar_html_seria():
    """
    Gera o HTML principal com análise da Serie A
    """
    print("=== GERANDO HTML SERIE A ===\n")
    
    # 1. Buscar últimos jogos de cada time
    print("1. Buscando últimos jogos do SofaScore...")
    dados_times = {}
    
    for team_name, team_id in SERIA_TEAM_IDS.items():
        # Evita duplicatas (usa nome completo, não abreviações)
        if team_name in ["Inter", "Como 1907", "Verona"]:
            continue
            
        print(f"   - {team_name}...")
        jogos = scrape_sofascore_seria_last5(team_id, team_name, debug=False)
    ...
    # Salvar arquivo
    with open("seria_analysis.html", "w", encoding="utf-8") as f:
        f.write(html)
...
<head>
    ...
    <title>Serie A 2025/2026 - Análise</title>
    <style>
        body {
